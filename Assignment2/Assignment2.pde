...
class Animal extends Node{
  ...
  Animal(Animal copying){
    animal_name = copying.animal_name;
    hair = copying.hair;
    feathers =  copying.feathers;
    eggs =  copying.eggs;
    milk = copying.milk;
    airborne = copying.airborne;
    aquatic = copying.aquatic;
    predator = copying.predator;
    toothed = copying.toothed;
    backbone = copying.backbone;
    breathes = copying.breathes;
    venomous = copying.venomous;
    fins = copying.fins;
    legs = copying.legs;
    tail = copying.tail;
    domestic = copying. domestic;
    catsize = copying.catsize;
    type = copying.type;
    
    child1 = copying.child1;
    child2 = copying.child2;
...
int findIndexOfClusterAnimalName(String nameOf){
  int thisIs = 0;
  for(int counter = 0; counter < cluster.size(); counter++){
    if(getAnimalName(cluster.get(counter)) == nameOf){
       thisIs = counter;
    }
  }
  return thisIs;
...
void removeDistanceBetween(Animal an1, Animal an2, ArrayList<Distance> thisList)
{
  for(int counter = 0; counter < thisList.size(); counter++){
    if(((thisList.get(counter).a1 == an1) && (thisList.get(counter).a2 == an2)) || ((thisList.get(counter).a1 == an2) && (thisList.get(counter).a2 == an1))){
      thisList.remove(counter);
      counter++;
    }
  }
}
